...
function processAll() {
    ...
    for (const tag of filteredTags) {
        ...
        newBaseTags.push({
            ...
            popularity: getTagPopularity(tag) // NEW: Store popularity
        });
    }
...
function displayTags() {
    ...
    if (sortValue === 'confidence') {
        let sortedTags = [...visibleTags].sort((a, b) => {
        ...
        container.dataset.groupName = 'popularity';
...
function createTagElement(tag) {
    ...
    // NEW: Show popularity in tooltip
...
// NEW: Fetch tag categories from Danbooru API
async function fetchDanbooruCategories() {
    if (!danbooruStatus) return;
    
    danbooruStatus.textContent = 'Fetching from Danbooru API...';
    ...
    try {
        ...
        while (hasMore && page <= 10) {
            const response = await fetch(`https://danbooru.donmai.us/tags.json?limit=1000&page=${page}&order=count`);
            ...
            danbooruStatus.textContent = `Fetched ${totalFetched} tags from Danbooru...`;
        ...
        danbooruStatus.textContent = `✓ Downloaded ${totalFetched} tags! Check your downloads folder.`;
        danbooruStatus.className = 'text-sm text-green-400';
        
        setTimeout(() => {
            danbooruStatus.textContent = '';
        }, 5000);
...
async function loadExternalData() {
    ...
    try {
        ...
        const [tagsResponse, mapResponse, popularityResponse] = await Promise.all([
            fetch(tagsUrl),
            fetch(mapUrl),
            fetch(popularityUrl).catch(() => null) // Optional file
        ]);
        ...
        // NEW: Load popularity data if available
        if (popularityResponse && popularityResponse.ok) {
            TAG_POPULARITY = await popularityResponse.json();
            console.log(`Loaded ${Object.keys(TAG_POPULARITY).length} tag popularity scores`);
        } else {
            console.log('tag_popularity.json not found - popularity sort will use zero weights');
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // NEW: Attach Danbooru fetch button
    if (fetchDanbooruBtn) {
        fetchDanbooruBtn.addEventListener('click', fetchDanbooruCategories);
    }
