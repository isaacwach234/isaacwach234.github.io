...
async function loadExternalData() {
    ...
    try {
        ...
        const tagsUrl = new URL('tags.json', document.baseURI);
        const mapUrl = new URL('tag_map.json', document.baseURI);
        
        tagsUrl.searchParams.set('t', timestamp);
        mapUrl.searchParams.set('t', timestamp);
        ...
        try {
            const popularityUrl = new URL('tag_popularity.json', document.baseURI);
            popularityUrl.searchParams.set('t', timestamp);
            const popularityResponse = await fetch(popularityUrl);
            if (popularityResponse.ok) {
                TAG_POPULARITY = await popularityResponse.json();
                console.log('Tag popularity data loaded successfully.');
            } else {
                throw new Error(`Local popularity file not found (status: ${popularityResponse.status}).`);
            }
        } catch (error) {
            console.warn(error.message);
            console.warn('Popularity sort will be unavailable. Make sure tag_popularity.json is in the root directory.');
        }
        ...
        tagOutput.innerHTML = `<div class="text-red-400 text-center py-12"><strong>Fatal Error:</strong> Could not load essential data files.<br><small>${error.message}</small><br><small>Please ensure 'tags.json' and 'tag_map.json' are in the correct location and accessible.</small></div>`;
    }
}

function displayTags() {
    if (!tagCategorizer) { 
        tagOutput.innerHTML = '<div class="text-gray-500 italic text-center py-12">Initializing...</div>';
        return; 
    }
    ...
    if (baseTags.length === 0) {
        ...
        updateHiddenCategoriesBanner(); updateStats(); updatePromptPreview(); refreshFavoriteIndicators(); destroySortableInstances();
        return;
    }
    ...
    if (sortValue === 'danbooru' || sortValue === 'smart') {
        ...
        if (sortValue === 'az') tagsToDisplay.sort((a, b) => a.original.localeCompare(b.original));
            ...
            if (Object.keys(TAG_POPULARITY).length === 0) {
                tagOutput.innerHTML = '<div class="text-amber-300 text-center py-12">Popularity data (tag_popularity.json) is not loaded. This sort option is unavailable.</div>';
                updateHiddenCategoriesBanner(); updateStats(); updatePromptPreview(); refreshFavoriteIndicators(); destroySortableInstances();
                return;
            }
            tagsToDisplay.sort((a, b) => {
                const popA = TAG_POPULARITY[a.original.toLowerCase().replace(/ /g, '_')] || 0;
                const popB = TAG_POPULARITY[b.original.toLowerCase().replace(/ /g, '_')] || 0;
                return popB - popA;
            });
        }
        const container = document.createElement('div');
...
document.addEventListener('DOMContentLoaded', () => {
    // --- SAFE INITIALIZATION ---
    // These actions do not depend on external data and will run immediately.
    initializeToken();
    ...
    if (howToPanel) { 
        ...
        if (storedState === 'true') { howToPanel.setAttribute('open', ''); } 
        else if (storedState === 'false') { howToPanel.removeAttribute('open'); } 
        howToPanel.addEventListener('toggle', () => { localStorage.setItem('danbooru-howto-open', howToPanel.open); }); 
    }
    ...
    // Attach all event listeners so the UI is responsive.
    ...
    // Now, load the data. This will not block the UI from appearing.
    displayTags(); // Show "Initializing..." message
    loadExternalData().then(() => {
        // Once data is loaded, do the first real render.
        processAll();
    });
});

document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && categoryPickerModal?.classList.contains('active')) { closeCategoryPicker(); } });
Object.assign(window, { processAll, suggestCoherentTags, randomizeTags, optimizeOrder, exportTags, importTags, copyPromptAsJson, clearAll, toggleSettingsPanel, showTokenSettings, hideTokenSettings, saveToken, testToken, exportSettings, importSettings, resetToDefaults, submitCategoryUpdate });