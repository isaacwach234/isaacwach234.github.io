    ...
    let TAG_DATABASE = [], gitHubPat = null, tagCategorizer, tagIdCounter = 0, taxonomyConfig = {};
    ...
    async function loadExternalData() {
        ...
        try {
            ...
            const [tagsResponse, mapResponse, taxonomyResponse] = await Promise.all([
                fetch(`tags.json?${timestamp}`),
                fetch(`tag_map.json?${timestamp}`),
                fetch(`assets/data/tag_taxonomy.json?${timestamp}`)
            ]);
            ...
            if (taxonomyResponse.ok) {
                ...
                console.warn('Failed to load taxonomy config, falling back to defaults:', taxonomyResponse.statusText);
                taxonomyConfig = {};
            }
            const fallbackOrder = ['Artists', 'Quality', 'Composition', 'Characters', 'Subject & Creatures', 'Species & Race', 'Copyright & Franchise', 'Emotion & Expression', 'Interaction', 'Face', 'Eyes', 'Hair', 'Body Parts', 'Attire', 'Accessories', 'Held Items & Objects', 'Actions & Poses', 'Camera & Perspective', 'Setting & Environment', 'Background Elements', 'Weather & Atmosphere', 'Lighting & Effects', 'Rendering', 'Post-processing', 'Style & Meta', 'Meta'];
            tagCategorizer = new EnhancedTagCategorizer(tagMap, TAG_DATABASE, fallbackOrder, taxonomyConfig);
            ...
            errorDiv.innerHTML = `<strong>Error loading core data:</strong> ${error.message}. The application cannot start. Please check the console (F12) for details and ensure 'tags.json', 'tag_map.json', and the taxonomy files are in the expected locations.`;
    ...
    window.stripTagWeights = stripTagWeights;
    ...
    function stripTagWeights() {
        const rawInput = tagInput.value;
        const sanitize = (segment) => {
            let cleaned = segment.trim();
            if (!cleaned) return '';
            cleaned = cleaned.replace(/^\(([^:()]+?):\d+(?:\.\d+)?\)$/i, '$1');
            cleaned = cleaned.replace(/^\[([^:()\[\]]+?):\d+(?:\.\d+)?\]$/i, '$1');
            let iterations = 0;
            while (iterations < 6 && cleaned.length > 1) {
        ...
        let changed = false;
        const strippedPieces = rawInput.split(',').map(piece => {
            const trimmed = piece.trim();
            const cleaned = sanitize(piece);
            if (trimmed && cleaned !== trimmed) changed = true;
            return cleaned;
        }).filter(Boolean);

        if (!strippedPieces.length && !baseTags.length) {
            copyMessage.textContent = 'No tags to strip.';
            setTimeout(() => copyMessage.textContent = '', 2000);
            return;
        }

        const sanitizedString = strippedPieces.join(', ');
        if (sanitizedString.length || rawInput.trim() || baseTags.length) {
            tagInput.value = sanitizedString;
            enableWeightingToggle.checked = false;
            processAll();
            baseTags.forEach(tag => {
                if (tag.weighted !== tag.original) changed = true;
        ...
        copyMessage.textContent = changed ? 'Removed tag weighting syntax.' : 'No weighting syntax detected.';
        setTimeout(() => copyMessage.textContent = '', 2000);
    }
    ...
    function optimizeOrder() {
        const categoryPriority = {
            ...
            'Subject & Creatures': 4,
            'Species & Race': 5,
            'Copyright & Franchise': 6,
            'Emotion & Expression': 7,
            'Interaction': 8,
            'Face': 9,
            'Eyes': 10,
            'Hair': 11,
            ...
            'Attire': 13,
            'Accessories': 14,
            'Held Items & Objects': 15,
            'Actions & Poses': 16,
            'Camera & Perspective': 17,
            'Setting & Environment': 18,
            'Background Elements': 19,
