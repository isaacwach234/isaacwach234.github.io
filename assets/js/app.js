...
class EnhancedTagCategorizer {
    constructor(tagMapsBySource, tagListsBySource, categoryOrderBySource, metadataBySource, options = {}) {
        this.sourceMaps = {};
        ...
        this.metadataSources = metadataBySource || {};
        ...
        this.categoryOrder = [];
        this.metadataAliasMap = {};
        this.metadataTagHints = {};
        this.metadataPrefixRules = [];
        this.metadataSuffixRules = [];
        this.metadataKeywordBuckets = [];
        this.metadataCompoundRules = [];
    ...
    resolveMetadataAlias(tag) {
        const normalized = this.normalizeTagKey(tag);
        let current = normalized;
        let guard = 0;
        while (this.metadataAliasMap[current] && guard < 12) {
            const next = this.metadataAliasMap[current];
    ...
    applyMetadata(metadata = {}) {
        ...
        if (metadata.aliases) {
            Object.entries(metadata.aliases).forEach(([alias, canonical]) => {
                const normalizedAlias = this.normalizeTagKey(alias);
                const normalizedCanonical = this.normalizeTagKey(canonical);
                if (!normalizedAlias || !normalizedCanonical) return;
                this.metadataAliasMap[normalizedAlias] = normalizedCanonical;
            });
        }
        if (metadata.tagHints) {
            Object.entries(metadata.tagHints).forEach(([tag, hint]) => {
                const normalizedTag = this.normalizeTagKey(tag);
                if (!normalizedTag || !hint?.category) return;
                registerCategory(hint.category);
                this.metadataTagHints[normalizedTag] = {
                    category: hint.category,
                    confidence: hint.confidence || 1,
                    source: hint.source || 'Metadata hint'
                };
            });
        }
        if (Array.isArray(metadata.prefixRules)) {
            this.metadataPrefixRules = metadata.prefixRules
                .map(rule => ({
                    prefix: this.normalizeTagKey(rule.prefix || ''),
                    category: rule.category,
                    confidence: rule.confidence || 1,
                    source: rule.source || 'Metadata prefix'
                }))
                .filter(rule => rule.prefix && rule.category);
            this.metadataPrefixRules.forEach(rule => registerCategory(rule.category));
        }
        if (Array.isArray(metadata.suffixRules)) {
            this.metadataSuffixRules = metadata.suffixRules
                .map(rule => ({
                    suffix: this.normalizeTagKey(rule.suffix || ''),
                    category: rule.category,
                    confidence: rule.confidence || 1,
                    source: rule.source || 'Metadata suffix'
                }))
                .filter(rule => rule.suffix && rule.category);
            this.metadataSuffixRules.forEach(rule => registerCategory(rule.category));
        }
        if (Array.isArray(metadata.keywordBuckets)) {
            this.metadataKeywordBuckets = metadata.keywordBuckets
                .map(bucket => ({
                    keywords: Array.isArray(bucket.keywords) ? bucket.keywords.map(keyword => this.normalizeTagKey(keyword)).filter(Boolean) : [],
                    category: bucket.category,
                    confidence: bucket.confidence || 1,
                    match: bucket.match || 'any',
                    source: bucket.source || 'Metadata keywords'
                }))
                .filter(bucket => bucket.keywords.length && bucket.category);
            this.metadataKeywordBuckets.forEach(bucket => registerCategory(bucket.category));
        }
        if (Array.isArray(metadata.compoundRules)) {
            this.metadataCompoundRules = metadata.compoundRules
                .map(rule => ({
                    terms: Array.isArray(rule.terms) ? rule.terms.map(term => this.normalizeTagKey(term)).filter(Boolean) : [],
                    category: rule.category,
                    confidence: rule.confidence || 1,
                    source: rule.source || 'Metadata compound'
                }))
                .filter(rule => rule.terms.length && rule.category);
            this.metadataCompoundRules.forEach(rule => registerCategory(rule.category));
        }
    }

    applyMetadataScores(tag, words, addScore) {
        if (!tag) return;
        const normalizedTag = this.normalizeTagKey(tag);
        const hint = this.metadataTagHints[normalizedTag];
        if (hint && hint.category) {
            addScore(hint.category, hint.confidence || 1, hint.source || 'Metadata');
        }
        this.metadataPrefixRules.forEach(rule => {
            if (rule.prefix && normalizedTag.startsWith(rule.prefix)) {
                addScore(rule.category, rule.confidence || 1, rule.source || 'Metadata prefix');
            }
        });
        this.metadataSuffixRules.forEach(rule => {
            if (rule.suffix && normalizedTag.endsWith(rule.suffix)) {
                addScore(rule.category, rule.confidence || 1, rule.source || 'Metadata suffix');
            }
        });
        if (this.metadataKeywordBuckets.length) {
            const wordSet = new Set(words.map(word => this.normalizeTagKey(word)));
            this.metadataKeywordBuckets.forEach(bucket => {
                if (!bucket.keywords.length) return;
                const matchType = (bucket.match || 'any').toLowerCase();
                const matches = bucket.keywords.map(keyword => normalizedTag.includes(keyword) || wordSet.has(keyword));
                const matched = matchType === 'all' ? matches.every(Boolean) : matches.some(Boolean);
                if (matched) {
                    addScore(bucket.category, bucket.confidence || 1, bucket.source || 'Metadata keywords');
                }
            });
        }
        this.metadataCompoundRules.forEach(rule => {
            if (!rule.terms.length) return;
            const matched = rule.terms.every(term => normalizedTag.includes(term));
            if (matched) {
                addScore(rule.category, rule.confidence || 1, rule.source || 'Metadata compound');
            }
        });
    }
    ...
    applySource(source) {
        ...
        const metadataForSource = this.metadataSources[source] || this.metadataSources.default || {};
        this.applyMetadata(metadataForSource);
    ...
    categorizeEnhanced(tagString) {
        const normalizedInput = this.normalizeTagKey(tagString);
        const canonicalTag = this.resolveMetadataAlias(normalizedInput);
        const primaryCategory = this.primaryIndex[normalizedInput] || this.primaryIndex[canonicalTag];
        if (primaryCategory) {
            const directSource = this.primaryIndex[normalizedInput] ? 'Primary' : 'Primary (alias)';
            return { category: primaryCategory, source: directSource, confidence: 1.0 };
        }
        const words = canonicalTag.split(/[_:]/).filter(Boolean);
        ...
        this.applyMetadataScores(canonicalTag, words, addScore);
        for (const pattern of this.characterPatterns) {
            if (pattern.test(canonicalTag)) addScore('Characters', 1.2, 'Character pattern');
        }
        if (canonicalTag.startsWith('rating:') || this.ratingKeywords.has(canonicalTag)) {
        ...
        Object.entries(this.e621KeywordHints).forEach(([e621Category, keywords]) => {
            ...
            keywords.forEach(keyword => {
                if (canonicalTag.includes(keyword)) addScore(mappedCategory, 0.85, `E621 hint '${keyword}'`);
            });
        });
        Object.entries(this.e621SuffixHints).forEach(([suffix, mappedCategory]) => {
            if (canonicalTag.endsWith(suffix)) addScore(mappedCategory, 0.8, `E621 suffix '${suffix}'`);
        });
        ...
        this.ontologyRegexRules.forEach(rule => {
            if (rule.regex.test(canonicalTag)) addScore(rule.category, rule.weight, rule.reason);
        });
        this.ontologyComboRules.forEach(rule => {
            if (rule.contains.every(token => canonicalTag.includes(token))) addScore(rule.category, rule.weight, rule.reason);
        });
        for (const prefix in this.patternIndex.starts) {
            if (canonicalTag.startsWith(prefix)) addScore(this.patternIndex.starts[prefix], 1.0, `Prefix '${prefix}'`);
        }
        for (const suffix in this.patternIndex.ends) {
            if (canonicalTag.endsWith(suffix)) addScore(this.patternIndex.ends[suffix], 1.0, `Suffix '${suffix}'`);
        }
        if (scoreMap.size === 0) {
    ...
    categorizeOriginal(tagString) {
        const normalizedInput = this.normalizeTagKey(tagString);
        const canonicalTag = this.resolveMetadataAlias(normalizedInput);
        if (this.primaryIndex[normalizedInput]) return { category: this.primaryIndex[normalizedInput], source: 'Primary' };
        if (this.primaryIndex[canonicalTag]) {
            const label = normalizedInput === canonicalTag ? 'Primary' : 'Primary (alias)';
            return { category: this.primaryIndex[canonicalTag], source: label };
        }
        if (canonicalTag.includes('(') && canonicalTag.includes(')')) {
            const seriesMatch = canonicalTag.match(/\(([^)]+)\)/);
        ...
        for (const prefix in this.patternIndex.starts) if (canonicalTag.startsWith(prefix)) return { category: this.patternIndex.starts[prefix], source: 'Pattern (Prefix)' };
        for (const suffix in this.patternIndex.ends) if (canonicalTag.endsWith(suffix)) return { category: this.patternIndex.ends[suffix], source: 'Pattern (Suffix)' };
        const words = canonicalTag.split(/[_:]/).filter(Boolean);
...
async function loadExternalData() {
    ...
    try {
        ...
        const [tagsResponse, mapResponse, metadataResponse, e621Response, ontologyResponse, e621MapResponse, e621TagsResponse] = await Promise.all([
            fetch(`data/tags.json?${timestamp}`),
            fetch(`data/tag_map.json?${timestamp}`),
            fetch(`data/tag_metadata.json?${timestamp}`).catch(() => null),
        ...
        let metadataBySource = {};
        if (metadataResponse && metadataResponse.ok) {
            metadataBySource = await metadataResponse.json();
        }
        let e621TagMap = {};
        ...
        tagCategorizer = new EnhancedTagCategorizer({
            danbooru: tagMap,
            e621: e621TagMap
        }, TAG_DATABASES, categoryOrderBySource, metadataBySource, {
            e621Taxonomy,
            ontology,
