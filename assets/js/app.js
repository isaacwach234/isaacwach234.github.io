...
class EnhancedTagCategorizer {
    constructor(tagMapsBySource, tagListsBySource, categoryOrderBySource, options = {}) {
        ...
        this.metadataBySource = options.metadataBySource || {};
    ...
    applySource(source) {
        ...
        this.metadataImplications = {};
        this.metadataTagTypes = {};
        this.metadataTypeCategoryMap = {};
        this.metadataKeywordIndex = {};
        this.metadataPatternRules = [];
        this.metadataStopWords = new Set();
        ...
        if (this.metadataBySource[source]) {
            this.applyMetadata(this.metadataBySource[source]);
        }
    ...
    applyMetadata(metadata = {}) {
        ...
        this.metadataKeywordIndex = {};
        this.metadataPatternRules = [];
        this.metadataStopWords = new Set((metadata.stopwords || []).map(word => this.normalizeTag(word)));
        ...
        Object.entries(metadata.type_map || {}).forEach(([type, categories]) => {
            const normalizedType = this.normalizeTag(type);
            const list = Array.isArray(categories) ? categories : [categories];
            this.metadataTypeCategoryMap[normalizedType] = list.filter(Boolean);
            list.forEach(registerCategory);
        });

        Object.entries(metadata.keyword_categories || {}).forEach(([category, keywords]) => {
            const normalizedKeywords = (keywords || []).map(keyword => this.normalizeTag(keyword)).filter(Boolean);
            normalizedKeywords.forEach(keyword => {
                if (!this.metadataKeywordIndex[keyword]) this.metadataKeywordIndex[keyword] = new Set();
                this.metadataKeywordIndex[keyword].add(category);
            });
        ...
        Object.entries(metadata.tag_metadata || {}).forEach(([tag, rawEntry]) => {
            const entry = coerceEntry(rawEntry);
            const normalizedTag = this.normalizeTag(tag);
            if (!normalizedTag) return;
            const category = entry.category || entry.primary_category;
            if (category) {
                this.metadataIndex[normalizedTag] = {
                    category,
            ...
            const declaredType = entry.type || entry.tag_type;
            if (declaredType) {
                this.metadataTagTypes[normalizedTag] = this.normalizeTag(declaredType);
            }
            if (Array.isArray(entry.aliases)) {
                entry.aliases.forEach(alias => {
                    const aliasNormalized = this.normalizeTag(alias);
                    if (aliasNormalized) this.metadataAliasIndex[aliasNormalized] = normalizedTag;
                });
            }
            if (Array.isArray(entry.implies)) {
                this.metadataImplications[normalizedTag] = entry.implies
                    .map(imp => this.normalizeTag(imp))
                    .filter(Boolean);
            }
        });
        ...
        Object.entries(metadata.aliases || {}).forEach(([alias, canonical]) => {
            const aliasNormalized = this.normalizeTag(alias);
            const canonicalNormalized = this.normalizeTag(canonical);
            if (aliasNormalized && canonicalNormalized) {
                this.metadataAliasIndex[aliasNormalized] = canonicalNormalized;
            }
        });

        Object.entries(metadata.implications || {}).forEach(([tag, implied]) => {
            const normalizedTag = this.normalizeTag(tag);
            const normalizedImplied = (implied || []).map(item => this.normalizeTag(item)).filter(Boolean);
            if (!normalizedTag || normalizedImplied.length === 0) return;
            if (!this.metadataImplications[normalizedTag]) this.metadataImplications[normalizedTag] = [];
            this.metadataImplications[normalizedTag].push(...normalizedImplied);
        });

        Object.entries(metadata.tag_types || {}).forEach(([tag, type]) => {
            const normalizedTag = this.normalizeTag(tag);
            const normalizedType = this.normalizeTag(type);
            if (normalizedTag && normalizedType) {
                this.metadataTagTypes[normalizedTag] = normalizedType;
            }
        });

        (metadata.patterns || []).forEach(pattern => {
            if (!pattern?.regex || !pattern.category) return;
            try {
                const regex = new RegExp(pattern.regex, pattern.flags || 'i');
                this.metadataPatternRules.push({
                    regex,
                    category: pattern.category,
                    weight: typeof pattern.weight === 'number' ? pattern.weight : 0.9,
                    reason: pattern.reason || 'Metadata pattern'
                });
                registerCategory(pattern.category);
            } catch (error) {
                console.warn('Invalid metadata regex skipped:', pattern.regex, error);
            }
        });
    }

    lookupMetadataCategory(tag) {
        const normalizedTag = this.normalizeTag(tag);
        if (!normalizedTag) return null;
        const direct = this.metadataIndex[normalizedTag];
        if (direct?.category) {
            return {
                category: direct.category,
                source: direct.source || 'Metadata',
                confidence: typeof direct.confidence === 'number' ? direct.confidence : 1
            };
        }
        const aliasTarget = this.metadataAliasIndex[normalizedTag];
        if (aliasTarget && this.metadataIndex[aliasTarget]?.category) {
            const entry = this.metadataIndex[aliasTarget];
            return {
                category: entry.category,
                source: 'Metadata Alias',
                confidence: typeof entry.confidence === 'number' ? Math.min(entry.confidence, 0.95) : 0.95
            };
        }
        return null;
    }
    ...
    buildHeuristicIndexes(allTags) {
        ...
        (allTags || []).forEach(tag => {
            const normalizedTag = this.normalizeTag(tag);
            const metadataCategory = this.metadataIndex?.[normalizedTag]?.category;
            const category = this.primaryIndex[tag] || metadataCategory;
            if (!category) return;
            const words = normalizedTag
                .split(/[_:]/)
                .filter(Boolean)
                .filter(word => !this.metadataStopWords.has(word));
            if (words.length > 1) {
                words.forEach(word => {
    ...
    categorizeEnhanced(tagString) {
        ...
        const metadataDirect = this.lookupMetadataCategory(normalizedTag);
        if (metadataDirect?.category) {
        ...
        const filteredWords = words.filter(word => !this.metadataStopWords.has(word));
        const metadataType = this.metadataTagTypes[normalizedTag];
        if (metadataType && this.metadataTypeCategoryMap[metadataType]) {
            this.metadataTypeCategoryMap[metadataType].forEach(category => {
                addScore(category, 1.05, `Metadata type '${metadataType}'`);
            });
        }
        (this.metadataImplications[normalizedTag] || []).forEach(impliedTag => {
            const implied = this.lookupMetadataCategory(impliedTag);
            if (implied?.category) addScore(implied.category, 0.95, `Implied by '${normalizedTag}'`);
        });
        filteredWords.forEach(word => {
            ...
            const metadataCategories = this.metadataKeywordIndex[word];
            if (metadataCategories) {
                metadataCategories.forEach(category => addScore(category, 0.85, `Metadata keyword '${word}'`));
            }
        });
        Object.entries(this.metadataKeywordIndex).forEach(([keyword, categories]) => {
            if (!keyword.includes('_')) return;
            if (normalizedTag.includes(keyword)) {
                categories.forEach(category => addScore(category, 0.75, `Metadata phrase '${keyword}'`));
            }
        });
        ...
        this.metadataPatternRules.forEach(rule => {
            if (rule.regex.test(normalizedTag)) addScore(rule.category, rule.weight, rule.reason);
        });
    ...
    categorizeOriginal(tagString) {
        ...
        const metadataDirect = this.lookupMetadataCategory(tag);
        if (metadataDirect?.category) return metadataDirect;
