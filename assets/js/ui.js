...
const SMART_SORT_MODES = new Set(['smart', 'flow', 'illustrious']);
    ...
    async function loadExternalData() {
        ...
        try {
            ...
            const bundledCatalog = catalogResponse.ok ? await catalogResponse.json() : {};
            const cached = loadCachedCatalog();
            state.catalogCacheInfo = cached;
            const combinedCatalog = { ...bundledCatalog, ...(cached?.data || {}) };
            ...
            state.tagCatalog = combinedCatalog;
            rebuildCategorizerWithCatalog({ reprocess: false });
            state.latestCatalogPayload = cached?.data || combinedCatalog;
